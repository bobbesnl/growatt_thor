...
### Improvements
- **Removed confusing Host field from setup**: The OCPP server always binds to `0.0.0.0` (all network interfaces of the Home Assistant host), making the Host field ineffective. Removing it prevents users from accidentally entering the THOR's IP address, which would cause the integration to fail silently.
